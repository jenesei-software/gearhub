...
- **Logitech через HID++**: устройства за приёмниками Unifying/Bolt находятся по connection-нотификациям
  приёмника (sub_id `0x41`) — даже когда устройство спит. Заряд читается по HID++ 2.0
  (`0x1004`/`0x1001`/`0x1000`) и HID++ 1.0 (регистры `0x0D`/`0x07`), а также из нотификаций самих устройств.
...
- [x] Подписка на connection-нотификации (`0x41`): устройства видны даже когда спят (тип + WPID).
- [ ] Logitech: чтение имени и точного заряда, когда устройство активно (сейчас — тип и WPID).
- [ ] G435 Lightspeed (Centurion-протокол): донгл не отвечает, когда наушники выключены — нужна проверка
